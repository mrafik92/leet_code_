/**
LeetCode Problem 2690: House Robber IV
...
pub fn min_capability(nums: Vec<i32>, k: i32) -> i32 {
  fn check(nums: &[i32], capability: i32, number_houses: i32) -> bool {
    let mut right = 0;
    let mut count = 0;
    while right < nums.len() {
      if nums[right] <= capability {
        count += 1;
        right += 2;
      } else {
        right += 1;
      }
      if count >= number_houses {
        return true;
      }
    }
    false
  ...
  let mut ng = *nums.iter().min().unwrap();
...
mod tests {
  ...
  fn test_min_capability() {
    assert_eq!(min_capability(vec![2, 3, 5, 9], 2), 9);
    assert_eq!(min_capability(vec![2, 7, 9, 3, 1], 2), 9);
  }
}
